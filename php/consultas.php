...
function validarLogin($link, $user, $pass, $tipo)
{
    ...
    $campoCorreo = ($tipo == "Paciente") ? "correo_electronico" : "correo_eletronico"; // el correo en la db en la tabla doctores es correo_eletronico
    ...
    if (mysqli_num_rows($resultado) == 1) {
        ...
        if (password_verify($pass, $row['clave'])) {
            ...
            $updateQuery = "UPDATE $tabla SET session_token = ? WHERE $campoId = ?";
            $stmtUpdate = mysqli_prepare($link, $updateQuery);
            mysqli_stmt_bind_param($stmtUpdate, "si", $sessionToken, $row[$campoId]);
